...
public class ConfectionStress extends ConfigBase {
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setCapacity(double value) {
        return builder -> {
            assertFromCreateCasing(builder);
            ResourceLocation id = Confectioneering.location(builder.getName());
    ...
    private static void assertFromCreateCasing(BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals(Confectioneering.MOD_ID)) {
            throw new IllegalStateException("Non-Confectioneering blocks cannot be added to Confectioneerings config.");
        }
    }
